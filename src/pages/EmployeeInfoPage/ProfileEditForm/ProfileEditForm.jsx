...
const ProfileEditForm = ({ userData, closeFormHandle, setMyUserInfo }) => {
	...
	useEffect(() => {
		...
		setValue("phone_number", phone_number ? phoneMask(phone_number) : "");
		setValue("email", email);
		setValue("birthday", birthday ? dayjs(birthday).format("DD.MM.YYYY") : "");

		setValue("medical_exam.id", medical_exam?.id || 0);
		setValue("medical_exam.number", medical_exam?.number || "");
		setValue(
			"medical_exam.expiration_date",
			dateMask(medical_exam?.expiration_date)
		);
		setValue("medical_exam.file", medical_exam?.file || "");

		for (let i = 0; i < trainings.length; i++) {
			const { id, category_id, expiration_date, number, file } = trainings[i];
			trainingsAppend({
				itemId: id,
				category_id,
				expiration_date: dateMask(expiration_date),
				number,
				file,
			});
		}
		...
		setValue(
			"protective_equipment_card.id",
			protective_equipment_card?.id || 0
		);
		setValue(
			"protective_equipment_card.number",
			protective_equipment_card?.number || ""
		);
		setValue(
			"protective_equipment_card.expiration_date",
			dateMask(protective_equipment_card?.expiration_date)
		);
		setValue(
			"protective_equipment_card.file",
			protective_equipment_card?.file || ""
		);

		for (let i = 0; i < tools.length; i++) {
			toolsAppend({ title: tools[i].title });
		}
	}, []);
	...
	const onSubmit = useCallback(
		async (data) => {
			...
			try {
				const newUserDataRes = await usersService.patchUser(
					userData.id,
					formData
				);
				setMyUserInfo(newUserDataRes.data);
				closeFormHandle();
			} catch (e) {
				console.log(e);
			}
		},
		[closeFormHandle, filesToUpload, setMyUserInfo, userData]
	);
